...
# Read the CSV file
data_0 <- read.csv("C:/Users/Caro/Desktop/Multiple_Testing/episodes/data/DEG_pvalues.csv")

# Check the first few rows of data_0
head(data_0)

# Histogram of p-values
ggplot(data_0, aes(x = pvalue)) +
...
sum(is.na(data_0$pvalue))
# Exclude NA values
data_0 <- na.omit(data_0)  # Removes rows with NA values
...
# Calculate significant genes based on p-values
alpha <- 0.05

data_0$pvalue <- as.numeric(as.character(data_0$pvalue))
significant_hits <- data_0$pvalue < alpha
...
alpha <- 0.05
bonferroni_threshold <- alpha / length(data_0$pvalue)
significant_bonferroni <- data_0$pvalue < bonferroni_threshold
...
p_adjusted <- p.adjust(data_0$pvalue, method = "BH")
